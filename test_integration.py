    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            response = await client.post(
                'http://localhost:8000/api/plan-trip', json=test_request
            )
            ...
            if response.status_code == 200:
                ...
                if flights:
                    flight = flights[0]
                    print(
                        f'   Sample: {flight.get("airline", "N/A")} - ${flight.get("price", "N/A")}'
                    )
                print()
                ...
                if hotels:
                    for i, hotel in enumerate(hotels, 1):
                        ...
                        print(
                            f'      Price: ${hotel.get("price_per_night", "N/A")}/night'
                        )
                        print(
                ...
                if activities:
                    ...
                    sights = [
                        a
                        for a in activities
                        if 'sight' in a.get('type', '').lower()
                    ]
                    experiences = [
                        a
                        for a in activities
                        if 'sight' not in a.get('type', '').lower()
                    ]
                    ...
                    if sights:
                        ...
                        for sight in sights:
                            ...
                            print(
                                f'      • {sight.get("name", "N/A")} - {price_text} ({sight.get("duration", "N/A")})'
                            )
                    ...
                    if experiences:
                        ...
                        for activity in experiences:
                            print(
                                f'      • {activity.get("name", "N/A")} - ${activity.get("price", "N/A")} ({activity.get("duration", "N/A")})'
                            )

                    if not sights and not experiences:
                        print(
                            '   Activities found but type classification unclear:'
                        )
                        for activity in activities[:3]:
                            print(
                                f'      • {activity.get("name", "N/A")} - ${activity.get("price", "N/A")}'
                            )
                            print(
                ...
                if schedule:
                    for day in schedule[:2]:  # Show first 2 days
                        ...
                        print(
                            f'   Day {day.get("day_number", "?")}: {len(day_activities)} activities'
                        )
                        for act in day_activities[:2]:
                            print(
                                f'      • {act.get("name", "N/A")} - ${act.get("price", "N/A")}'
                            )
...
async def main():
    ...
    if api_ok:
        ...
        print(
            'The travel agent is successfully integrating hotels and activities.'
        )
    else:
