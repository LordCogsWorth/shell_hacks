...
from travel_apis import ActivityBookingAPI, HotelBookingAPI, HotelSearchParams
...
async def test_hotels():
    ...
    print(
        f'📅 Check-in: {test_params.check_in}, Check-out: {test_params.check_out}'
    )
    ...
    try:
        ...
        if hotels:
            ...
            for i, hotel in enumerate(hotels[:3], 1):
                ...
                print(
                    f'   Amenities: {", ".join(hotel.get("amenities", [])[:3])}'
                )
        else:
...
async def test_activities():
    ...
    try:
        activities = await activity_api.search_activities(
            destination, max_budget, preferences
        )
        ...
        if activities:
            ...
            experiences = [
                a for a in activities if 'sight' not in a.get('category', '')
            ]
            ...
            if sights:
                ...
                for i, sight in enumerate(sights[:3], 1):
                    price_text = (
                        f'${sight.get("price", "N/A")}'
                        if sight.get('price', 0) > 0
                        else 'Free'
                    )
                    print(f'   {i}. {sight.get("name", "N/A")} - {price_text}')
                    print(
                        f'      Type: {sight.get("type", "N/A")} | Duration: {sight.get("duration", "N/A")} | Rating: {sight.get("rating", "N/A")}'
                    )
            ...
            if experiences:
                ...
                for i, activity in enumerate(experiences[:3], 1):
                    print(
                        f'   {i}. {activity.get("name", "N/A")} - ${activity.get("price", "N/A")}'
                    )
                    print(
                        f'      Type: {activity.get("type", "N/A")} | Duration: {activity.get("duration", "N/A")} | Rating: {activity.get("rating", "N/A")}'
                    )
                    print(
                        f'      {activity.get("description", "N/A")[:100]}...'
                    )
        else:
...
async def test_api_integration():
    ...
    try:
        ...
        if result.get('activities'):
            ...
            print(
                f'🎯 Sample activity: {activity.get("name", "N/A")} - ${activity.get("price", "N/A")}'
            )
...
async def test_environment_setup():
    ...
    try:
        ...
        print(
            f'🔑 Amadeus API Key: {"✅ Found" if amadeus_key else "❌ Missing"}'
        )
        print(
            f'🔑 Amadeus Secret: {"✅ Found" if amadeus_secret else "❌ Missing"}'
        )
        print(
            f'🔑 TripAdvisor Key: {"✅ Found" if tripadvisor_key else "❌ Missing"}'
        )
...
async def main():
    ...
    if all([env_ok, hotel_ok, activities_ok, integration_ok]):
        ...
        if not env_ok:
            print('\n🔧 Environment Issues:')
            print(
                '   - Check your .env file has AMADEUS_API_KEY and AMADEUS_API_SECRET'
            )
        ...
        if not hotel_ok:
            ...
            print(
                '   - Hotel API may be using mock data (which is OK for testing)'
            )
        ...
        if not activities_ok:
            ...
            print('   - Check network connectivity')
            print(
                '   - Activities API may be using mock data (which is OK for testing)'
            )
